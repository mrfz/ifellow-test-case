...
.PHONY: test
test:
	poetry run pytest -c pyproject.toml --cov-report=html --cov=ifellow_test_case tests/
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 ifellow_test_case tests
...
check-safety:
	...
	poetry run bandit -ll --recursive ifellow_test_case tests
